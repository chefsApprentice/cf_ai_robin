...
export class DatabaseService {
  ...
  async updateImageAltText(instanceId: string, altText: string): Promise<void> {
    try {
      await this.db
        .prepare('UPDATE Images SET ImageAltText = ? WHERE InstanceID = ?')
        .bind(altText, instanceId)
        .run();
    } catch (error) {
      throw new Error(`Failed to update image altText: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  async getImageAltText(instanceId: string): Promise<string | null> {
    try {
      const result = await this.db
        .prepare('SELECT ImageAltText FROM Images WHERE InstanceID = ?')
        .bind(instanceId)
        .first<ImageRecord>();

      return result?.ImageAltText || null;
    } catch (error) {
      throw new Error(`Failed to get image alt text: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
}
