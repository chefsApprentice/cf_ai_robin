...
export default function UploadsTable({ uploadedImages }: UploadsTableProps) {
  ...
  const [aiAltTextStates, setAiAltTextStates] = useState<{ [key: string]: 'pending' | 'sent' | 'sending' | null }>({});
  ...
  const handleAiAltTextRequest = async (instanceId: string, wantAltText: boolean) => {
    // Set loading state immediately
    setAiAltTextStates(prev => ({ ...prev, [instanceId]: 'sending' }));

    console.log('Sending AI alt text request for instanceId:', instanceId, 'with wantAltText:', wantAltText);

    try {
      const response = await fetch(`${API_BASE_URL}/approval-for-ai-alttext`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({
          instanceId,
          approved: wantAltText
        })
      });

      if (!response.ok) {
        throw new Error('Failed to send AI alttext request');
      }

      setAiAltTextStates(prev => ({ ...prev, [instanceId]: 'sent' }));
    } catch (error) {
      console.error('Error sending AI alttext request:', error);
      // Reset state on error
      setAiAltTextStates(prev => ({ ...prev, [instanceId]: null }));
    }
  };
      ...
      <table className="w-full">
        <thead className="border-b border-white/10">
          <tr>
            ...
            <th className="px-8 py-4 text-left text-lg font-light text-white/80">AI Image Alt Text</th>
            <th className="px-8 py-4 text-left text-lg font-light text-white/80">Generate Image Alt Text?</th>
          </tr>
        </thead>
        <tbody className="divide-y divide-white/10">
          {[...uploadedImages].reverse().map((image, index) => (
            <tr key={index} className="group">
              ...
              <td className="px-8 py-6 text-white/70 group-hover:text-white/90">
                {image.aiAltText ? image.aiAltText : 'N/A'}
              </td>
              <td className="px-8 py-6">
                {aiTagStates !== null ?
                  <>
                    {
                      aiAltTextStates[image.instanceId] === 'sent' ? (
                        <span className="text-green-400">Event sent to workflow</span>
                      ) : aiAltTextStates[image.instanceId] === 'sending' ? (
                        <span className="text-blue-400">Sending event to workflow...</span>
                      ) : (
                        <div className="flex gap-2">
                          <button
                            onClick={() => handleAiAltTextRequest(image.instanceId, true)}
                            className="px-4 py-2 bg-blue-500/80 hover:bg-blue-500 text-white rounded-lg transition-colors"
                          >
                            Yes
                          </button>
                          <button
                            onClick={() => handleAiAltTextRequest(image.instanceId, false)}
                            className="px-4 py-2 bg-gray-500/80 hover:bg-gray-500 text-white rounded-lg transition-colors"
                          >
                            No
                          </button>
          ...
          {uploadedImages.length === 0 && (
            <tr>
              <td colSpan={6} className="px-8 py-12 text-center text-xl text-white/50">
